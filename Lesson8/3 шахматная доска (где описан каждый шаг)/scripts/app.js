let app = {
    ...
    insertFiguresOnDesk() {
        ...
        for (let i = 0; i < positions.length; i++) {
            // 5.1 получаем ссылку на тег td с атрибутами data-rownum и data-colchar, 
            //которым будут соответствовать свойства coordRow и coordCol из массива positions
            let cell = document.querySelector(`[data-rownum="${positions[i].coordRow}"][data-colchar="${positions[i].coordCol}"]`);
            // 5.2 из массива positions, для текущего объекта по индексу i, 
            //передаем название фигуры, затем цвет+"Figure"
     ...
     */
    generateBoard() {
        // 3. создаем переменную board и присваиваем ей пустую строку, 
        //    в ней будет храниться разметка игрового поля в виде строки
        let board="";
        // 3.1 создаем переменную rowStartWithColor, в которой укажем, 
        //что линия на игровом поле будет начинаться с белого цвета т.е. присваиваем строку "white"
    ...
    generateRow(startWithColor, rowNum) {
        ...
        for (let i = 0; i < this.config.cols.length; i++) {
            ...
            if (currentColorClass=='white') {
                // 2.5 в переменную field пишем результат вызова функции this.generateField, 
                //которой передаем цвет ячейки игрового поля в виде строки, 
                //затем номер строки игровой доски в виде числа, последним аргументом передаем this.config.cols[i]
